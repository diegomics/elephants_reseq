...
mkdir -p $QC_DIR
FASTQC_JOB=$(sbatch --array=1-$LENGTH --output=${QC_DIR}/%x.%A_%a.out --error=${QC_DIR}/%x.%A_%a.err slurm/ReSeq_FastQC.job)
...
mkdir -p $TRIM_DIR
FASTP_JOB=$(sbatch --dependency=afterok:$FASTQC_JOB_ID --array=1-$LENGTH --output=${TRIM_DIR}/%x.%A_%a.out --error=${TRIM_DIR}/%x.%A_%a.err slurm/ReSeq_Fastp.job)
